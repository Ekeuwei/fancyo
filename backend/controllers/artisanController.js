...
exports.registerArtisan = catchAsyncErrors( async (req, res, next) =>{
    ...
    const { firstName, lastName, displayName, phoneNumber, contact, category, email, password } = req.body;
...
exports.updatePassword = catchAsyncErrors( async (req, res, next) => {
    ...
    const isPasswordMatched = await artisan.compareArtisanPassword(req.body.oldPassword);
...
exports.updateProfile = catchAsyncErrors( async (req, res, next) => {
    ...
    const artisan = await Artisan.findByIdAndUpdate(req.artisan.id, newArtisanData, {
        new: true,
        runValidators: true,
...
// Get all artisans => /api/v1/artisans
exports.allArtisans = catchAsyncErrors( async (req, res, next) => {
...
exports.getArtisanDetails = catchAsyncErrors( async (req, res, next) => {
    ...
    if(!artisan){
        return next(new ErrorHandler(`Artisan does not found with id: ${req.params.id}`, 404));
    }
...
exports.updateArtisan = catchAsyncErrors( async (req, res, next) => {
    ...
    const artisan = await Artisan.findByIdAndUpdate(req.params.id, newArtisanData, {
        new: true,
        runValidators: true,
        useFindAndModify: false
    });
